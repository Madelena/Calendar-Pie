...
Run as an ordinary user in the graphical session. This temporary server isolates the static renderer test; no Node.js installation or development server is needed for that test. The full application deployment uses the managed application and kiosk services described in the [deployment guide](DEPLOYMENT.md#deploy-to-a-raspberry-pi).
